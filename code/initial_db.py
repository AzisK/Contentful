...
def get_table_status():
    is_table_result = execute(f"SELECT table_status FROM {TABLE_STATUS_TABLE} WHERE table_name = '{ORGANIZATIONS_TABLE}'")
...
def get_initial_data():
    jsons = read_data()
    tuples = [(record["organization_key"], record["organization_name"], record["created_at"]) for record in jsons]
    return tuples
